...
moqui.webrootVue=new Vue({el:"#apps-root",data:{basePath:"",linkBasePath:"",currentPathList:[],extraPathList:[],activeSubscreens:[],currentParameters:{},bodyParameters:null,navMenuList:[],navHistoryList:[],navPlugins:[],notifyHistoryList:[],lastNavTime:Date.now(),loading:0,activeContainers:{},moquiSessionToken:"",appHost:"",appRootPath:"",userId:"",locale:"en",notificationClient:null,qzVue:null},methods:{setUrl:function(a,b){this.bodyParameters=b;$(".modal.in").modal("hide");a=this.getLinkPath(a);
if(this.currentLinkUrl===a&&a!==this.linkBasePath)this.reloadSubscreens();else{var c=a,d=c.indexOf("://");if(0<=d){d=c.indexOf("/",d+3);if(-1===d)return;c=c.slice(d)}c=c.split("?");this.extraPathList=[];this.currentSearch=1<c.length&&0<c[1].length?c[1]:"";this.currentPath=c[0];c=this.currentSearch;if((c=this.currentPath+(0<c.length?"?"+c:""))&&0!==c.length){console.info("current URL changing to "+c);this.lastNavTime=Date.now();this.activeContainers={};var e=this,c=this.appRootPath&&this.appRootPath.length&&
0===c.indexOf(this.appRootPath)?this.appRootPath+"/menuData"+c.slice(this.appRootPath.length):"/menuData"+c;$.ajax({type:"GET",url:c,dataType:"text",error:moqui.handleAjaxError,success:function(a){var b=null;try{b=JSON.parse(a)}catch(c){console.info("Error parson menu list JSON: "+c)}b&&moqui.isArray(b)&&(e.navMenuList=b)}});window.history.pushState(null,this.ScreenTitle,a)}}},setParameters:function(a){a&&(this.$root.currentParameters=$.extend({},this.$root.currentParameters,a));this.$root.reloadSubscreens()},
addSubscreen:function(a){a.pathIndex=this.activeSubscreens.length;a.loadActive();this.activeSubscreens.push(a)},reloadSubscreens:function(){var a=this.currentPathList,b=this.activeSubscreens;console.info("reloadSubscreens currentPathList "+JSON.stringify(this.currentPathList));if(0===a.length&&0<b.length)b.splice(1),b[0].loadActive();else for(var c=0;c<b.length&&!(c>=a.length);c++)b[c].loadActive()&&b.splice(c+1)},goPreviousScreen:function(){for(var a=this.currentPath,b=this.navHistoryList,c,d=0;d<
b.length;d++)if(0>b[d].pathWithParams.indexOf(a)){c=b[d];break}c&&c.pathWithParams&&c.pathWithParams.length&&this.setUrl(c.pathWithParams)},addContainer:function(a,b){this.activeContainers[a]=b},reloadContainer:function(a){var b=this.activeContainers[a];b?b.reload():console.error("Container with ID "+a+" not found, not reloading")},loadContainer:function(a,b){var c=this.activeContainers[a];c?c.load(b):console.error("Container with ID "+a+" not found, not loading url "+b)},addNavPlugin:function(a){var b=
this;moqui.loadComponent(this.appRootPath+a,function(a){b.navPlugins.push(a)})},addNotify:function(a,b){var c=this.notifyHistoryList.slice(0),d=new Date,e=d.getHours();10>e&&(e="0"+e);d=d.getMinutes();10>d&&(d="0"+d);for(c.unshift({message:a,type:b,time:e+":"+d});25<c.length;)c.pop();this.notifyHistoryList=c},switchDarkLight:function(){var a=$("body");a.toggleClass("bg-dark");a.toggleClass("bg-light");a=a.hasClass("bg-dark")?"bg-dark":"bg-light";$.ajax({type:"POST",url:this.appRootPath+"/apps/setPreference",
