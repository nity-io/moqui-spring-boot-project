 ...
 */
package io.nity.moqui.web.sample
...
@CompileStatic
class SampleUserFacadeImpl implements UserFacade {
...
//    protected WebExecutionContext eci
    ...
    private static org.apache.shiro.mgt.SecurityManager internalSecurityManager

    SampleUserFacadeImpl() {
//        this.eci = (WebExecutionContext) MoquiWeb.getExecutionContext()
        pushUser(null)
    }

    public WebExecutionContext getEci(){
        WebExecutionContext eci = (WebExecutionContext) MoquiWeb.getExecutionContext()
        return  eci
    }
    ...
    Subject makeEmptySubject() {
        if (session != null) {
            ...
            return getSecurityManager().createSubject(wsc)
        } else {
            return getSecurityManager().createSubject(new DefaultSubjectContext())
        }
    }
    ...
    String getPasswordHashType() {
        MNode passwordNode = eci.getFactory().confXmlRoot.first("user-facade").first("password")
    ...
    void initFromHttpRequest(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (eci.getMessage().hasError()) request.setAttribute("moqui.login.error", "true")

        this.visitId = session.getAttribute("moqui.visitId")
        // NOTE: only tracking Visitor and Visit if there is a WebFacadeImpl in place
        if (eci.getWeb() != null && !this.visitId && !eci.getSkipStats()) {
            MNode serverStatsNode = eci.getFactory().getServerStatsNode()
            ScreenUrlInfo sui = ScreenUrlInfo.getScreenUrlInfo(eci.getScreen(), request)
            ...
            if (!isJustContent && !"false".equals(serverStatsNode.attribute('visit-enabled'))) {
                ...
                InetAddress address = eci.getFactory().getLocalhostAddress()
    ...
    @Override ArrayList<Timestamp> getPeriodRange(String period, String poffset, String pdate) {
        ...
        java.sql.Date sqlDate = (pdate != null && !pdate.isEmpty()) ? eci.getL10n().parseDate(pdate, null) : null
        ...
        if (inputFieldsMap.get(baseName + "_period")) {
            ...
            if (fromValue && fromValue instanceof CharSequence) {
                if (fromValue.length() < 12)
                    rangeList.set(0, eci.getL10n().parseTimestamp(fromValue.toString() + " 00:00:00.000", "yyyy-MM-dd HH:mm:ss.SSS"))
                else
                    rangeList.set(0, eci.getL10n().parseTimestamp(fromValue.toString(), null))
            ...
            if (thruValue && thruValue instanceof CharSequence) {
                if (thruValue.length() < 12)
                    rangeList.set(1, eci.getL10n().parseTimestamp(thruValue.toString() + " 23:59:59.999", "yyyy-MM-dd HH:mm:ss.SSS"))
                else
                    rangeList.set(1, eci.getL10n().parseTimestamp(thruValue.toString(), null))
    ...
    @Override boolean loginUser(String username, String password) {
        if (username == null || username.isEmpty()) {
            eci.getMessage().addError(eci.l10n.localize("No username specified"))
            return false
        }
        if (password == null || password.isEmpty()) {
            eci.getMessage().addError(eci.l10n.localize("No password specified"))
            return false
        }
    ...
    String getLoginKeyHashType() {
        MNode loginKeyNode = eci.getFactory().confXmlRoot.first("user-facade").first("login-key")
    ...
    static class UserInfo {
        final SampleUserFacadeImpl ufi
        ...
        UserInfo(SampleUserFacadeImpl ufi, String username) {
            this.ufi = ufi
            setInfo(username)
        }
