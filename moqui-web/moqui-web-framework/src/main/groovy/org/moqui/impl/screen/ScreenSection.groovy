...
class ScreenSection {
    ...
    ScreenSection(WebExecutionContextFactory ecfi, MNode sectionNode, String location) {
        ...
        if (sectionNode.first("condition")?.first() != null) {
            ...
            condition = new XmlAction(ecfi.getResource().getFactory(), sectionNode.first("condition").first(), location + ".condition")
        }
        // prep actions
        if (sectionNode.hasChild("actions")) {
            actions = new XmlAction(ecfi.getResource().getFactory(), sectionNode.first("actions"), location + ".actions")
