...
class RestApi {
    ...
    @SuppressWarnings("GrFinalVariableAccess") protected final ServiceExecutionContextFactory ecfi
    @SuppressWarnings("GrFinalVariableAccess") final Cache<String, ResourceNode> rootResourceCache

    RestApi(ServiceExecutionContextFactory ecfi) {
        this.ecfi = ecfi
        rootResourceCache = ecfi.getCache().getLocalCache("service.rest.api")
    ...
    synchronized void loadRootResourceNode(String name) {
        ...
        for (String location in this.ecfi.getComponentBaseLocations().values()) {
            ResourceReference serviceDirRr = this.ecfi.getResource().getLocationReference(location + "/service")
    ...
    List<ResourceNode> getFreshRootResources() {
        ...
        Cache.Entry<String, ResourceNode> entry = null
    ...
    static abstract class MethodHandler {
        ServiceExecutionContextFactory ecfi
        String method
        PathNode pathNode
        String requireAuthentication
        MethodHandler(MNode methodNode, PathNode pathNode, ServiceExecutionContextFactory ecfi) {
            this.ecfi = ecfi
    ...
    static class MethodService extends MethodHandler {
        ...
        MethodService(MNode methodNode, MNode serviceNode, PathNode pathNode, ServiceExecutionContextFactory ecfi) {
        ...
        RestResult run(List<String> pathList, ExecutionContext ec) {
            ...
            try {
                Map result = ecfi.getService().sync().name(serviceName).parameters(ec.context).call()
        ...
        void addToSwaggerMap(Map<String, Object> swaggerMap, Map<String, Map<String, Object>> resourceMap) {
            ServiceDefinition sd = ecfi.getService().getServiceDefinition(serviceName)
    ...
    static class MethodEntity extends MethodHandler {
        ...
        MethodEntity(MNode methodNode, MNode entityNode, PathNode pathNode, ServiceExecutionContextFactory ecfi) {
            super(methodNode, pathNode, ecfi)
        ...
        RestResult run(List<String> pathList, ExecutionContext ec) {
            ...
            try {
                ...
                if (operation == 'one') {
                    EntityFind ef = entityFacade.find(entityName).searchFormMap(ec.context, null, null, null, false)
                    if (masterName) {
                    ...
                    Map result = serviceFacade.sync().name(operation, entityName).parameters(ec.context).call()
                    return new RestResult(result, null)
                } else {
        ...
        Map<String, Object> getRamlMap(Map<String, Object> typesMap) {
            ...
            for (int i = 0; i < remainingPkFields.size(); i++) {
                FieldInfo fi = ed.getFieldInfo(remainingPkFields.get(i) as String)
    ...
    static abstract class PathNode {
        ...
        PathNode(MNode node, PathNode parent, ServiceExecutionContextFactory ecfi, boolean isId) {
            this.ecfi = ecfi
            this.parent = parent
        ...
        RestResult runByMethod(List<String> pathList, ExecutionContext ec) {
            Method getMethod = ec.class.getMethod("getWeb")
            Object webFacade = getMethod.invoke(ec)

            getMethod = webFacade.class.getMethod("getRequest")

            HttpServletRequest request = (HttpServletRequest) getMethod.invoke(webFacade)
    ...
    static class ResourceNode extends PathNode {
        ResourceNode(MNode node, PathNode parent, ServiceExecutionContextFactory ecfi) {
    ...
    static class IdNode extends PathNode {
        IdNode(MNode node, PathNode parent, ServiceExecutionContextFactory ecfi) {
