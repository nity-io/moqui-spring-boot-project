...
class ScreenTree {
    ...
    ScreenTree(WebExecutionContextFactory ecfi, ScreenDefinition sd, MNode treeNode, String location) {
        this.ecfi = ecfi
        this.sd = sd
    ...
    static class TreeNode {
        ...
        TreeNode(ScreenTree screenTree, MNode treeNodeNode, String location) {
            ...
            if (treeNodeNode.hasChild("condition") && treeNodeNode.first("condition").children) {
                ...
                condition = new XmlAction(screenTree.ecfi.getResource().getFactory(), treeNodeNode.first("condition").children[0], location + ".condition")
            }
            // prep actions
            if (treeNodeNode.hasChild("actions")) actions = new XmlAction(screenTree.ecfi.getResource().getFactory(), treeNodeNode.first("actions"), location + ".actions")
    ...
    static class TreeSubNode {
        ...
        TreeSubNode(ScreenTree screenTree, MNode treeSubNodeNode, String location) {
            ...
            if (treeSubNodeNode.hasChild("condition") && treeSubNodeNode.first("condition").children) {
                // the script is effectively the first child of the condition element
                condition = new XmlAction(screenTree.ecfi.getResource().getFactory(), treeSubNodeNode.first("condition").children[0], location + ".condition")
            }
            // prep actions
            if (treeSubNodeNode.hasChild("actions")) actions =
                    new XmlAction(screenTree.ecfi.getResource().getFactory(), treeSubNodeNode.first("actions"), location + ".actions")
        }
    }
}
