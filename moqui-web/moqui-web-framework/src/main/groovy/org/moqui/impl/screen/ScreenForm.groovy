...
class ScreenForm {
    ...
    protected WebExecutionContext ec
    ...
    ScreenForm(WebExecutionContextFactory ecfi, ScreenDefinition sd, MNode baseFormNode, String location) {
        this.ecfi = ecfi
        this.ec = ecfi.getExecutionContext() as WebExecutionContext
        this.sd = sd
        ...
        try {
            EntityList dbFormLookupList = ecfi.getEntity().find("DbFormLookup")
    ...
    boolean isDisplayOnly() {
        ContextStack cs = ecfi.getExecutionContext().getContext()
    ...
    void initForm(MNode baseFormNode, MNode newFormNode) {
        ...
        if (baseFormNode.attribute("extends")) {
            ...
            if (isDynamic) extendsForm = ecfi.getResource().expand(extendsForm, "")
            ...
            if (extendsForm.contains("#")) {
                ...
                if (screenLocation == sd.getLocation()) {
                    ...
                    ScreenDefinition esd = ecfi.getScreen().getScreenDefinition(screenLocation)
        ...
        for (MNode formSubNode in baseFormNode.children) {
            if (formSubNode.name == "field") {
                ...
                if (isDynamic) serviceName = ecfi.getResource().expand(serviceName, "")
                ServiceDefinition serviceDef = ecfi.getService().getServiceDefinition(serviceName)
                if (serviceDef != null) {
                ...
                if (ecfi.getService().isEntityAutoPattern(serviceName)) {
                    EntityDefinition ed = ecfi.getEntity().getEntityDefinition(ServiceDefinition.getNounFromName(serviceName))
                    if (ed != null) {
                ...
                if (isDynamic) entityName = ecfi.getResource().expand(entityName, "")
        ...
        // prep row-actions
        if (newFormNode.hasChild("row-actions")) rowActions = new XmlAction(ecfi.getResource().getFactory(), newFormNode.first("row-actions"), location + ".row_actions")
    }
    ...
    void setSubFieldValidateAttrs(MNode newFormNode, String transitionAttribute, String subFieldNodeName) {
        if (newFormNode.attribute(transitionAttribute)) {
            ...
            if (ti != null && ti.getSingleServiceName()) {
                ...
                ServiceDefinition sd = ecfi.getService().getServiceDefinition(singleServiceName)
                if (sd != null) {
                ...
                } else if (ecfi.getService().isEntityAutoPattern(singleServiceName)) {
    ...
    List<MNode> getDbFormNodeList() {
        ...
        try {
            ...
            EntityList dbFormLookupList = this.ecfi.getEntity().find("DbFormLookup")
    ...
    static MNode getDbFormNode(String formId, WebExecutionContextFactory ecfi) {
        MNode dbFormNode = (MNode) ecfi.getScreen().dbFormNodeByIdCache.get(formId)

        if (dbFormNode == null) {

            boolean alreadyDisabled = ecfi.getExecutionContext().getArtifactExecution().disableAuthz()
            try {
                EntityValue dbForm = ecfi.getEntity().fastFindOne("moqui.screen.form.DbForm", true, false, formId)
                ...
                EntityList dbFormFieldList = ecfi.getEntity().find("moqui.screen.form.DbFormField").condition("formId", formId)
                ...
                for (EntityValue dbFormField in dbFormFieldList) {
                    ...
                    EntityList dbFormFieldAttributeList = ecfi.getEntity().find("moqui.screen.form.DbFormFieldAttribute")
                    ...
                    EntityList dbFormFieldOptionList = ecfi.getEntity().find("moqui.screen.form.DbFormFieldOption")
                            .condition([formId:formId, fieldName:fieldName] as Map<String, Object>).useCache(true).list()
                    EntityList dbFormFieldEntOptsList = ecfi.getEntity().find("moqui.screen.form.DbFormFieldEntOpts")
                            .condition([formId:formId, fieldName:fieldName] as Map<String, Object>).useCache(true).list()
                    EntityList combinedOptionList = new EntityListImpl(ecfi.getEntity())
                    ...
                    for (EntityValue optionValue in combinedOptionList) {
                        if (optionValue.getEntityName() == "moqui.screen.form.DbFormFieldOption") {
                            ...
                            EntityList dbFormFieldEntOptsCondList = ecfi.getEntity().find("moqui.screen.form.DbFormFieldEntOptsCond")
                            ...
                            EntityList dbFormFieldEntOptsOrderList = ecfi.getEntity().find("moqui.screen.form.DbFormFieldEntOptsOrder")
                                    .condition([formId:formId, fieldName:fieldName, sequenceNum:optionValue.sequenceNum])
                ...
                ecfi.getScreen().dbFormNodeByIdCache.put(formId, dbFormNode)
            } finally {
                if (!alreadyDisabled) ecfi.getExecutionContext().getArtifactExecution().enableAuthz()
            }
        }
    ...
    void addAutoServiceField(EntityDefinition nounEd, MNode parameterNode, String fieldType,
        ...
        if (parameterNode.attribute("entity-name")) fieldEd = ecfi.getEntity().getEntityDefinition(parameterNode.attribute("entity-name"))
    ...
    protected void addEntityFieldDropDown(MNode oneRelNode, MNode subFieldNode, EntityDefinition relatedEd,
        ...
        if (relatedEntityName == "moqui.basic.Enumeration") {
            recordCount = ecfi.getEntity().find("moqui.basic.Enumeration").condition("enumTypeId", title).disableAuthz().count()
        } else if (relatedEntityName == "moqui.basic.StatusItem") {
            recordCount = ecfi.getEntity().find("moqui.basic.StatusItem").condition("statusTypeId", title).disableAuthz().count()
        } else {
            recordCount = ecfi.getEntity().find(relatedEntityName).disableAuthz().count()
        }
    ...
    protected void expandFieldSubNode(MNode baseFormNode, MNode fieldNode, MNode fieldSubNode) {
        ...
        if (widgetNode.name == "auto-widget-service") {
            ...
            MNode widgetTemplatesNode = ecfi.getScreen().getWidgetTemplatesNodeByLocation(fileLocation)
    ...
    protected void addAutoWidgetServiceNode(MNode baseFormNode, MNode fieldNode, MNode fieldSubNode, MNode widgetNode) {
        ...
        if (serviceName.contains("#")) {
            EntityDefinition ed = ecfi.getEntity().getEntityDefinition(serviceName.substring(serviceName.indexOf("#")+1))
            if (ed != null) {
    ...
    protected void addAutoWidgetEntityNode(MNode baseFormNode, MNode fieldNode, MNode fieldSubNode, MNode widgetNode) {
        ...
        try {
            ed = ecfi.getEntity().getEntityDefinition(entityName)
        } catch (EntityException e) {
            // ignore, anticipating there may be no entity def
            if (logger.isTraceEnabled()) logger.trace("Ignoring entity exception, not necessarily an entity name: ${e.toString()}")
        }
    ...
    static LinkedHashMap<String, String> getFieldOptions(MNode widgetNode, ExecutionContext ec, WebExecutionContextFactory wecf) {
        ...
        for (int wci = 0; wci < widgetChildrenSize; wci++) {
            ...
            if ("entity-options".equals(childNode.name)) {
                ...
                EntityFind ef = wecf.getEntity().find(entityFindNode)
                ...
                if (ef.shouldCache()) {
                    ...
                    for (int k = 0; k < dateFilterListSize; k++) {
                        ...
                        EntityCondition dateEc = wecf.getEntity().conditionFactory.makeConditionDate(df.attribute("from-field-name") ?: "fromDate",
                                df.attribute("thru-field-name") ?: "thruDate",
                                (df.attribute("valid-date") ? wecf.getResource().expression(df.attribute("valid-date"), null) as Timestamp : ec.user.nowTimestamp))
                ...
                for (int i = 0; i < eliSize; i++) {
                    ...
                    addFieldOption(options, fieldNode, childNode, ev, ec, wecf)
                }
            } else if ("list-options".equals(childNode.name)) {
                Object listObject = wecf.getResource().expression(childNode.attribute('list'), null)
                if (listObject instanceof EntityListIterator) {
                    EntityListIterator eli
                    try {
                        eli = (EntityListIterator) listObject
                        EntityValue ev
                        while ((ev = eli.next()) != null) addFieldOption(options, fieldNode, childNode, ev, ec, wecf)
                    } finally {
                        eli.close()
                    }
                } else {
                    ...
                    for (Object listOption in listObject) {
                        if (listOption instanceof Map) {
                            addFieldOption(options, fieldNode, childNode, (Map) listOption, ec, wecf)
                        } else {
                            if (keyAttr != null || textAttr != null) {
                                addFieldOption(options, fieldNode, childNode, [entry:listOption], ec, wecf)
                            } else {
                ...
                String key = wecf.resource.expandNoL10n(childNode.attribute('key'), null)
                String text = wecf.resource.expand(childNode.attribute('text'), null)
    ...
    static void addFieldOption(LinkedHashMap<String, String> options, MNode fieldNode, MNode childNode, Map listOption,
                               ExecutionContext ec, WebExecutionContextFactory wecf) {
        ...
        try {
            ...
            if (keyAttr != null && keyAttr.length() > 0) {
                key = wecf.resource.expandNoL10n(keyAttr, null)
            ...
            if (text == null || text.length() == 0) {
                ...
                String value = wecf.resource.expand(text, null)
    ...
    static class FormInstance {
        ...
        private WebExecutionContextFactory ecfi
                ...
                if (isListForm) {
                    ...
                    if (aggregate != null && !aggregate.isEmpty()) {
                        ...
                        aggregateFieldMap.put(fieldName, new AggregateField(fieldName, af, isGroupBy, isSubList, showTotal,
                                ecfi.getResource().getGroovyClass(fieldNode.attribute("from"))))
                        if (isGroupBy) {
        ...
        MNode getFieldValidateNode(MNode subFieldNode) {
            ...
            if (validateService) {
                ServiceDefinition sd = ecfi.getService().getServiceDefinition(validateService)
                ...
                EntityDefinition ed = ecfi.getEntity().getEntityDefinition(validateEntity)
        ...
        boolean isListFieldHiddenAttr(MNode fieldNode) {
            ...
            if (hideAttr != null && hideAttr.length() > 0) {
                return ecfi.resource.condition(hideAttr, "")
            }
            return false
        }
        ...
        String getUserActiveFormConfigId(ExecutionContext ec) {
            EntityValue fcu = ecfi.getEntity().fastFindOne("moqui.screen.form.FormConfigUser", true, false, screenForm.location, ec.user.userId)
            ...
            EntityList fcugvList = ecfi.getEntity().find("moqui.screen.form.FormConfigUserGroupView")
        ...
        private ArrayList<ArrayList<MNode>> makeDbFormListColumnInfo(String formConfigId, ExecutionContext eci) {
            EntityList formConfigFieldList = ecfi.getEntity().find("moqui.screen.form.FormConfigField")
    ...
    static class FormListRenderInfo {
        ...
        Object getListObject(boolean aggregateList) {
            ...
            MNode entityFindNode = screenForm.entityFindNode
            if (entityFindNode != null) {
                EntityFindBase ef = (EntityFindBase) ecfi.getEntity().find(entityFindNode)
                ...
                if (useCache) for (MNode df in entityFindNode.children("date-filter")) {
                    ...
                    if (validDateAttr != null && !validDateAttr.isEmpty()) validDate = ecfi.getResource().expression(validDateAttr, "") as Timestamp
                ...
                ContextStack context = ecfi.getEci().getContext()
                ...
                listObject = ecfi.getResource().expression(listName, "")
            }
    ...
    static String processFormSavedFind(WebExecutionContext ec) {
        ...
        if (isDelete) {
            if (flf == null) { ec.getMessage().addError("Saved find with ID ${formListFindId} not found, not deleting"); return null }
        ...
        ScreenDefinition screenDef = ec.getScreen().getScreenDefinition(screenLocation)
