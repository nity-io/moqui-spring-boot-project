...
class ScreenRenderImpl implements ScreenRender {
    ...
    public final WebExecutionContext ec
    ...
    protected String webappName = (String) MoquiWeb.WEBAPP_NAME
    ...
    ScreenRenderImpl(ScreenFacadeImpl sfi) {
        this.sfi = sfi
        ec = (WebExecutionContext) sfi.ecfi.getExecutionContext()
    }
    ...
    @Override ScreenRender rootScreen(String rsLocation) {
        rootScreenLocation = rsLocation;
        return this
    }
    @Override
    ScreenRender rootScreenFromHost(String host) {
        return rootScreen(sfi.rootScreenFromHost(host, webappName))
    }
    ...
    void render(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (originalScreenPathNameList == null || originalScreenPathNameList.size() == 0) {
            ArrayList<String> pathList = webFacade.getPathInfoList()
            screenPath(pathList)
        }
    ...
    boolean sendJsonRedirect(UrlInstance fullUrl, Long renderStartTime) {
        if ("json".equals(screenUrlInfo.targetTransitionExtension) || request?.getHeader("Accept")?.contains("application/json")) {
            ...
            WebFacade webFacade = ec.getWeb()
            // send it
            webFacade.sendJsonResponse(responseMap)
    ...
    Map<String, Object> getBasicResponseMap() {
        ...
        if (webFacade.requestParameters != null) parms.putAll(webFacade.requestParameters)
        if (webFacade.requestAttributes != null) parms.putAll(webFacade.requestAttributes)
    ...
    protected void internalRender() {
        ...
        if (targetTransition != null) {
            ...
            if (request != null && targetTransition.hasActionsOrSingleService()) {
                WebExecutionContextFactory webExecutionContextFactory = (WebExecutionContextFactory) ec.getFactory()
                WebExecutionContextFactoryImpl.WebappInfo webappInfo = webExecutionContextFactory.getWebappInfo(webappName)
                ...
                if (request.getMethod().toLowerCase() != "get" && webappInfo != null && webappInfo.requireSessionToken &&
                    ...
                    WebFacade webFacade = ec.getWeb()
                    String passedToken = (String) webFacade.getParameters().get("moquiSessionToken")
                    if (!passedToken) passedToken = request.getHeader("moquiSessionToken") ?: request.getHeader("SessionToken")
                    String curToken = webFacade.getSessionToken()
            ...
            TransactionFacade transactionFacade = sfi.getEcfi().getTransaction()
            ...
            TemplateRenderer tr = sfi.ecfi.getResource().getTemplateRendererByLocation(fileResourceRef.location)
            ...
            String fileContentType = sfi.ecfi.getResource().getContentType(tr != null ? tr.stripTemplateExtension(fileName) : fileName)
            ...
            WebExecutionContextFactoryImpl.WebappInfo webappInfo = sfi.getEcfi().getWebappInfo(webappName)
            if (isBinary) {
                if (response != null) {
                ...
                if (tr != null) {
                    ...
                    String text = sfi.ecfi.getResource().getLocationText(fileResourceRef.location, true)
    ...
    protected ResponseItem recursiveRunTransition(boolean runPreActions) {
        ...
        ec.getArtifactExecution().pushInternal(aei, false, false)
        ...
        try {
            ...
            if ("anonymous-all".equals(requireAuthentication)) {
                ...
                ec.getArtifactExecution().setAnonymousAuthorizedView()
                loggedInAnonymous = ec.getUser().loginAnonymousIfNoUser()
            }
    ...
    protected void recursiveRunActions(boolean runAlwaysActions, boolean runPreActions) {
        ...
        ec.getArtifactExecution().pushInternal(aei, !activeScreenHasNext ? (!requireAuthentication || requireAuthentication == "true") : false, false)
    ...
    void doActualRender() {
        ArrayList<ScreenDefinition> screenPathDefList = screenUrlInfo.screenPathDefList
        int screenPathDefListSize = screenPathDefList.size()
        WebExecutionContextFactoryImpl.WebappInfo webappInfo = sfi.ecfi.getWebappInfo(webappName)
        ...
        boolean beganTransaction = screenUrlInfo.beginTransaction ? sfi.ecfi.getTransaction().begin(null) : false
        try {
            ...
            try {
                    ...
                    for (int i = screenUrlInfo.renderPathDifference; i < screenPathDefListSize; i++) {
                        ...
                        for (ScreenDefinition.ParameterItem pi in sd.getParameterMap().values()) {
                            ...
                            if (ObjectUtilities.isEmpty(parmValue)) {
                                ec.message.addError(resourceFacade.expand("Required parameter missing (${pi.name})","",[pi:pi]))
                ...
                if (aeiList) for (int i = (aeiList.size() - 1); i >= 0; i--) ec.artifactExecution.pop(aeiList.get(i) as ArtifactExecutionInfo)
            }
            ...
            sfi.ecfi.getTransaction().rollback(beganTransaction, errMsg, t)
            throw new RuntimeException(errMsg, t)
        } finally {
            // if we began a tx commit it
            if (beganTransaction && sfi.ecfi.getTransaction().isTransactionInPlace()) sfi.ecfi.getTransaction().commit()
        }
    }
    ...
    boolean checkWebappSettings(ScreenDefinition currentSd) {
        ...
        if ((requireAuthentication == null || requireAuthentication.length() == 0 || requireAuthentication == "true")
                && (userId == null || userId.length() == 0) && !ec.getUser().getLoggedInAnonymous()) {
    ...
    String renderSubscreen() {
        ...
        if (!getActiveScreenHasNext()) {
            if (screenUrlInfo.fileResourceRef != null) {
                ...
                sfi.ecfi.getResource().template(screenUrlInfo.fileResourceRef.location, writer)
                return ""
            } else {
    ...
    String renderText(String location, String isTemplateStr) {
        ...
        if (isTemplate) {
            ...
            cs.push()
            cs.put("sri", this)
            sfi.ecfi.getResource().template(location, writer)
            cs.pop()
            writer.flush()
            // NOTE: this returns a String so that it can be used in an FTL interpolation, but it always writes to the writer
            return ""
        } else {
            return sfi.ecfi.getResource().getLocationText(location, true) ?: ""
        }
    }
    ...
    ScreenUrlInfo buildUrlInfo(String subscreenPathOrig) {
        ResourceFacade resourceFacade = ec.getResource()
        String subscreenPath = subscreenPathOrig?.contains("\${") ? resourceFacade.expand(subscreenPathOrig, "") : subscreenPathOrig
    ...
    UrlInstance makeUrlByType(String origUrl, String urlType, MNode parameterParentNode, String expandTransitionUrlString) {
        ...
        String urlTypeExpanded = resourceFacade.expand(urlType, "")
        switch (urlTypeExpanded) {
            ...
            case "plain":
            default:
                String url = resourceFacade.expand(origUrl, "")
        ...
        if (parameterParentNode != null) {
            ...
            if (parameterMapStr != null && !parameterMapStr.isEmpty()) {
                Map ctxParameterMap = (Map) resourceFacade.expression(parameterMapStr, "")
    ...
    Object getContextValue(String from, String value) {
        ResourceFacade resourceFacade = ec.getResource()
        if (value) {
            return resourceFacade.expand(value, getActiveScreenDef().location, (Map) ec.getContext().get("_formMap"))
        } else if (from) {
            return resourceFacade.expression(from, getActiveScreenDef().location, (Map) ec.getContext().get("_formMap"))
        } else {
            return ""
        }
    }
    ...
    String pushSingleFormMapContext(String mapExpr) {
        ...
        ResourceFacade resourceFacade = ec.getResource()
        Map valueMap = null
        if (mapExpr != null && !mapExpr.isEmpty()) valueMap = (Map) resourceFacade.expression(mapExpr, null)
    ...
    String getFieldValueString(MNode widgetNode) {
        ...
        String strValue = ec.getL10n().format(obj, format)
        return strValue
    }
    ...
    String getFieldValuePlainString(MNode fieldNodeWrapper, String defaultValue) {
        ...
        if (ObjectUtilities.isEmpty(obj) && defaultValue != null && defaultValue.length() > 0)
            return resourceFacade.expandNoL10n(defaultValue, "")
    ...
    String getNamedValuePlain(String fieldName, MNode formNode) {
        ...
        if ("form-single".equals(formNode.name)) {
            ...
            Map valueMap = (Map) resourceFacade.expression(mapName, "")
        ...
        if (value == null) value = ec.getContext().getByString(fieldName)
    ...
    Object getFieldValue(MNode fieldNode, String defaultValue) {
        ...
        if ("form-single".equals(formNode.name)) {
            ...
            Map<String, Object> errorParameters = webFacade?.getErrorParameters()
            ...
            if (fromAttr != null && fromAttr.length() > 0) return resourceFacade.expression(fromAttr, null)
        ...
        String defaultStr = resourceFacade.expand(defaultValue, null)
    ...
    String getFieldEntityValue(MNode widgetNode) {
        ...
        EntityDefinition ed = sfi.ecfi.getEntity().getEntityDefinition(entityName)
        ...
        EntityValue ev = entityFacade.find(entityName).condition(keyFieldName, fieldValue)
        ...
        if (text != null && text.length() > 0) {
            ...
            try {
                value = resourceFacade.expand(text, null)
            } finally {
                ec.context.pop()
            }
        } else {
    ...
    protected String getDefaultText(MNode widgetNode) {
        ...
        if (defaultText != null && defaultText.length() > 0) {
            return resourceFacade.expand(defaultText, null)
        } else {
            return ""
        }
    }
    ...
    LinkedHashMap<String, String> getFieldOptions(MNode widgetNode) {
        LinkedHashMap<String, String> optsMap = ScreenForm.getFieldOptions(widgetNode, ec, sfi.getFactory())
    ...
    boolean addNodeParameters(MNode parameterParentNode, Map<String, Object> parameters) {
        ...
        for (int i = 0; i < doNodeList.size(); i++) {
            ...
            if (ObjectUtilities.isEmpty(contextVal) && ec.getContext().get("_formMap") != null)
                contextVal = ((Map) ec.getContext().get("_formMap")).get(doField)
    ...
    String getCurrentThemeId() {
        ...
        EntityFacade entityFacade = sfi.ecfi.getEntity()
        // see if there is a user setting for the theme
        String themeId = entityFacade.fastFindOne("moqui.security.UserScreenTheme", true, true, ec.getUser().userId, stteId)?.screenThemeId
    ...
    ArrayList<String> getThemeValues(String resourceTypeEnumId) {
        ...
        EntityList strList = sfi.ecfi.getEntity().find("moqui.screen.ScreenThemeResource")
    ...
    String getThemeIconClass(String text) {
        ...
        EntityList stiList = sfi.ecfi.getEntity().find("moqui.screen.ScreenThemeIcon")
    ...
    List<Map> getMenuData(ArrayList<String> pathNameList) {
        WebFacade webFacade = ec.getWeb()
        ResourceFacade resourceFacade = ec.getResource()
        if (!ec.user.userId) { webFacade.sendError(401, "Authentication required", null); return null }
        ScreenUrlInfo fullUrlInfo = ScreenUrlInfo.getScreenUrlInfo(this, rootScreenDef, pathNameList, null, 0)
        if (!fullUrlInfo.targetExists) { webFacade.sendError(404, "Screen not found for path ${pathNameList}", null); return null }
        UrlInstance fullUrlInstance = fullUrlInfo.getInstance(this, null)
        if (!fullUrlInstance.isPermitted()) { webFacade.sendError(403, "View not permitted for path ${pathNameList}", null); return null }
        ...
        for (int i = 0; i < (fullPathSize - 1); i++) {
            ...
            curScreen = ec.getScreen().getScreenDefinition(curSsi.location)
            ...
            for (int j = 0; j < menuItemsSize; j++) {
                ...
                Map itemMap = [name:subscreensItem.name, title:resourceFacade.expand(subscreensItem.menuTitle, ""),
        ...
        fullUrlInstance.addParameters(webFacade.getRequestParameters())
        ...
        if (lastTitle.contains('${')) lastTitle = resourceFacade.expand(lastTitle, "")
        ...
        if (extraPathList != null) {
            ...
            for (int i = 0; i < extraPathListSize; i++) extraPathList.set(i, StringUtilities.urlEncodeIfNeeded(extraPathList.get(i) as String))
        }
