...
class NotificationMessageImpl implements NotificationMessage, Externalizable {
    ...
    private transient WebExecutionContextFactory ecfiTransient = (WebExecutionContextFactory) null

    /** Default constructor for deserialization */
    NotificationMessageImpl() { }
    NotificationMessageImpl(WebExecutionContextFactory ecfi) { ecfiTransient = ecfi }

    WebExecutionContextFactory getEcfi() {
        //TODO
//        if (ecfiTransient == null) ecfiTransient = (WebExecutionContextFactory) MoquiWeb.getF getExecutionContextFactory()
        return ecfiTransient
    }
    EntityValue getNotificationTopic() {
        if (notificationTopic == null && topic != null && !topic.isEmpty())
            notificationTopic = ecfi.getEntity().fastFindOne("moqui.security.user.NotificationTopic", true, true, topic)
    ...
    @Override Set<String> getNotifyUserIds() {
        ...
        EntityFacade ef = ecfi.getEntity()
    ...
    @Override NotificationMessage send() {
        ...
        if (isPersistOnSend()) {
            ...
            ecfi.getTransaction().runRequireNew(null, "Error saving NotificationMessage", {
        ...
        if (localEmailTemplateId != null && !localEmailTemplateId.isEmpty()) {
            ...
            EntityList emailNotificationUsers = ecfi.getEntity().find("moqui.security.user.NotificationTopicUser")
            ...
            if (emailNotificationUsersSize > 0) {
                ...
                for (int i = 0; i < emailNotificationUsersSize; i++) {
                    ...
                    EntityValue userAccount = ecfi.getEntity().find("moqui.security.UserAccount")
                    ...
                    if (emailAddress) {
                        ...
                        ecfi.getService().async().name("org.moqui.impl.EmailServices.send#EmailTemplate")
    ...
    @Override NotificationMessage markSent(String userId) {
        ...
        try {
            ecfi.getEntity().makeValue("moqui.security.user.NotificationMessageUser")
    ...
    @Override NotificationMessage markViewed(String userId) {
        ...
        markViewed(notificationMessageId, userId, ecfi.getEci(), ecfi)
        return this
    }
    static Timestamp markViewed(String notificationMessageId, String userId, ExecutionContext ec, WebExecutionContextFactory ecfi) {
        boolean alreadyDisabled = ec.getArtifactExecution().disableAuthz()
        try {
            Timestamp recStamp = new Timestamp(System.currentTimeMillis())
            ecfi.entity.makeValue("moqui.security.user.NotificationMessageUser")
                    .set("userId", userId).set("notificationMessageId", notificationMessageId)
