...
class RestSchemaUtil {
    ...
    static Map<String, Object> getRamlTypeMap(EntityDefinition ed, boolean pkOnly, Map<String, Object> typesMap,
        ...
        ArrayList<String> allFields = pkOnly ? ed.getPkFieldNames() : ed.getAllFieldNames()
        for (int i = 0; i < allFields.size(); i++) {
            FieldInfo fi = ed.getFieldInfo(allFields.get(i) as String)
    ...
    static void handleEntityRestSchema(ExecutionContextImpl eci, List<String> extraPathNameList, String schemaUri, String linkPrefix,
        ...
        if (!eci.getUser().getUsername()) {
            ...
            webFacade.sendJsonError(HttpServletResponse.SC_UNAUTHORIZED, errorMessage)
            return
        }

        EntityFacade efi = eci.getEntity()
        ...
        if (extraPathNameList.size() == 0) {
            ...
            webFacade.sendTextResponse(jsonStr, "application/schema+json", "MoquiEntities.schema.json")
        } else {
            ...
            try {
                EntityDefinition ed = efi.getEntityDefinition(entityName)
                if (ed == null) {
                    webFacade.sendJsonError(HttpServletResponse.SC_BAD_REQUEST, "No entity found with name or alias [${entityName}]")
                    return
                }
                ...
                webFacade.sendTextResponse(jsonStr, "application/schema+json", "${entityName}.schema.json")
    ...
    static void handleEntityRestRaml(ExecutionContextImpl eci, List<String> extraPathNameList, String linkPrefix, String schemaLinkPrefix, boolean getMaster) {
        ...
        webFacade.sendTextResponse(yamlString, "application/raml+yaml", "MoquiEntities.raml")
    }

    static void handleEntityRestSwagger(ExecutionContext eci, List<String> extraPathNameList, String basePath, boolean getMaster) {
        WebFacade webFacade = eci.getWeb()
        if (extraPathNameList.size() == 0) {
            webFacade.sendJsonError(HttpServletResponse.SC_BAD_REQUEST, "No entity name specified in path (for all entities use 'all')")
            return
        }
        ...
        webFacade.response.addHeader("Access-Control-Allow-Origin", "*")
        webFacade.response.addHeader("Access-Control-Allow-Methods", "GET, POST, DELETE, PUT, PATCH, OPTIONS")
        webFacade.response.addHeader("Access-Control-Allow-Headers", "Content-Type, api_key, Authorization")

        String fullHost = WebFacadeImpl.makeWebappHost(webFacade.webappMoquiName, (WebExecutionContext) webFacade.getFactory().getEci(), webFacade, true)
        ...
        if (outputType == "application/json") {
            ...
            jb.call(swaggerMap)
            String jsonStr = jb.toPrettyString()
            webFacade.sendTextResponse(jsonStr, "application/json", "${filename}.swagger.json")
            ...
            webFacade.sendTextResponse(yamlString, "application/yaml", "${filename}.swagger.yaml")
        } else {
            webFacade.sendJsonError(HttpServletResponse.SC_BAD_REQUEST, "Output type ${outputType} not supported")
        }
    }
    ...
    static void handleServiceRestSwagger(ExecutionContextImpl eci, List<String> extraPathNameList, String basePath) {
        ...
        if (extraPathNameList.size() == 0) {
            webFacade.sendJsonError(HttpServletResponse.SC_BAD_REQUEST, "No root resource name specified in path")
            return
        }
        ...
        String scheme = fullHost.substring(0, fullHost.indexOf("://"))
        String hostName = fullHost.substring(fullHost.indexOf("://") + 3)
        Map swaggerMap = webFacade.getRestApi().getSwaggerMap(rootPathList, [scheme], hostName, basePath)
        if (outputType == "application/json") {
            JsonBuilder jb = new JsonBuilder()
            jb.call(swaggerMap)
            String jsonStr = jb.toPrettyString()
            webFacade.sendTextResponse(jsonStr, "application/json", "${filenameBase}swagger.json")
        } else if (outputType == "application/yaml") {
            DumperOptions options = new DumperOptions()
            options.setDefaultFlowStyle(DumperOptions.FlowStyle.BLOCK)
            // default: options.setDefaultScalarStyle(DumperOptions.ScalarStyle.PLAIN)
            ...
            webFacade.sendTextResponse(yamlString, "application/yaml", "${filenameBase}swagger.yaml")
        } else {
            webFacade.sendJsonError(HttpServletResponse.SC_BAD_REQUEST, "Output type ${outputType} not supported")
        }
    }
    ...
    static void handleServiceRestRaml(ExecutionContextImpl eci, List<String> extraPathNameList, String linkPrefix) {
        ...
        Map swaggerMap = webFacade.getRestApi().getRamlMap(rootResourceName, linkPrefix)
        DumperOptions options = new DumperOptions()
        options.setDefaultFlowStyle(DumperOptions.FlowStyle.BLOCK)
        // default: options.setDefaultScalarStyle(DumperOptions.ScalarStyle.PLAIN)
        options.setPrettyFlow(true)
        ...
        webFacade.sendTextResponse(yamlString, "application/raml+yaml", "${rootResourceName}.raml")
    }
}
