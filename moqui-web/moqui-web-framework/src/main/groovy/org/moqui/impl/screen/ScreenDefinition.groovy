...
class ScreenDefinition {
    ...
    ScreenDefinition(ScreenFacadeImpl sfi, MNode screenNode, String location) {
        ...
        WebExecutionContextFactory ecfi = sfi.ecfi
        ...
        if (screenNode.hasChild("always-actions"))
            alwaysActions = new XmlAction(ecfi.getResource().getFactory(), screenNode.first("always-actions"), location + ".always_actions")
        // transition
        ...
        for (MNode transitionInclNode in screenNode.children("transition-include")) {
            ScreenDefinition includeScreen = ecfi.getScreen().getScreenDefinition(transitionInclNode.attribute("location"))
        ...
        if (screenNode.hasChild("pre-actions"))
            preActions = new XmlAction(ecfi.getResource().getFactory(), screenNode.first("pre-actions"), location + ".pre_actions")
    ...
    void pullSectionInclude(MNode sectionNode) {
        ...
        ScreenDefinition includeScreen = sfi.getEcfi().getScreen().getScreenDefinition(location)
    ...
    void populateSubscreens() {
        ...
        if (locationRef.supportsAll()) {
            ...
            ResourceReference subscreensDirRef = sfi.ecfi.getResource().getLocationReference(subscreensDirStr)
        ...
        EntityFind subscreensItemFind = sfi.ecfi.getEntity().find("moqui.screen.SubscreensItem")
    ...
    static String getPrettyMenuName(String menuName, String location, WebExecutionContextFactory ecfi) {
        ...
        return ecfi.getEci().getL10n().localize(menuName)
    }
    ...
    void render(ScreenRenderImpl sri, boolean isTargetScreen) {
        ...
        sri.ec.getArtifactExecution().pushInternal(aei, isTargetScreen ?
        ...
        if ("anonymous-all".equals(requireAuthentication)) {
            sri.ec.getArtifactExecution().setAnonymousAuthorizedAll()
            loggedInAnonymous = sri.ec.getUser().loginAnonymousIfNoUser()
        } else if ("anonymous-view".equals(requireAuthentication)) {
            sri.ec.getArtifactExecution().setAnonymousAuthorizedView()
            loggedInAnonymous = sri.ec.getUser().loginAnonymousIfNoUser()
        }
        ...
        try {
            ...
            sri.ec.getArtifactExecution().pop(aei)
    ...
    ResourceReference getSubContentRef(List<String> pathNameList) {
        ...
        ResourceReference lastScreenRef = sfi.ecfi.getResource().getLocationReference(location)
    ...
    List<Map<String, Object>> getScreenDocumentInfoList() {
        String localeString = sfi.ecfi.getEci().getUser().getLocale().toString()
        ...
        EntityList list = sfi.ecfi.getEntity().find("moqui.screen.ScreenDocument").condition("screenLocation", location)
    ...
    static class ParameterItem {
        ...
        ParameterItem(MNode parameterNode, String location, WebExecutionContextFactory ecfi) {
    ...
    static class TransitionItem {
        ...
        TransitionItem(MNode transitionNode, ScreenDefinition parentScreen) {
            ...
            WebExecutionContextFactory ecfi = parentScreen.sfi.ecfi
            // parameter
            ...
            if (transitionNode.first("condition")?.first() != null) {
                ...
                condition = new XmlAction(parentScreen.sfi.ecfi.getResource().getFactory(), transitionNode.first("condition").first(), location + ".condition")
            }
            // allow both call-service and actions
            if (transitionNode.hasChild("actions")) {
                actions = new XmlAction(parentScreen.sfi.ecfi.getResource().getFactory(), transitionNode.first("actions"), location + ".actions")
            }
            if (transitionNode.hasChild("service-call")) {
                ...
                serviceActions = new XmlAction(parentScreen.sfi.ecfi.getResource().getFactory(), callServiceNode, location + ".service_call")
        ...
        void setAllParameters(List<String> extraPathNameList, WebExecutionContext ec) {
        ...
        ResponseItem run(ScreenRenderImpl sri) {
            ...
            ec.getArtifactExecution().pushInternal(aei, (!requireAuthentication || "true".equals(requireAuthentication)), true)
                    ...
                    if (ec.getService().isEntityAutoPattern(singleServiceName)) {
                        ...
                        if (ed != null) {
                            ...
                            for (int i = 0; i < fieldNameListSize; i++) {
                                ...
                                Object fieldValue = ec.getContext().getByString(fieldName)
                                if (fieldValue instanceof CharSequence) {
                                    String fieldString = fieldValue.toString()
                                    if (fieldString.contains("<")) {
                                        ec.getMessage().addValidationError(null, fieldName, singleServiceName,
    ...
    static class ResponseItem {
        ...
        ResponseItem(MNode responseNode, TransitionItem ti, ScreenDefinition parentScreen) {
            ...
            if (responseNode.first("condition")?.first() != null) {
                // the script is effectively the first child of the condition element
                condition = new XmlAction(parentScreen.sfi.ecfi.getResource().getFactory(), responseNode.first("condition").first(),
                        location + ".condition")
            }
        ...
        boolean checkCondition(WebExecutionContext ec) { return condition ? condition.checkCondition(ec) : true }

        String getType() { return type }
        String getUrl() { return parentScreen.sfi.ecfi.getResource().expandNoL10n(url, "") }
        ...
        Map expandParameters(List<String> extraPathNameList, WebExecutionContext ec) {
    ...
    static class SubscreensItem {
        ...
        String getDefaultTitle() {
            WebExecutionContextFactory ecfi = parentScreen.sfi.ecfi
            ResourceReference screenRr = ecfi.getResource().getLocationReference(location)
        ...
        boolean isValidInCurrentContext() {
            ExecutionContext eci = parentScreen.sfi.getEcfi().getEci()
    ...
    static class SubscreensItemComparator implements Comparator<SubscreensItem> {
        ...
        int compare(SubscreensItem ssi1, SubscreensItem ssi2) {
            ...
            ResourceFacade rf = ssi1.parentScreen.sfi.ecfi.getResource()
