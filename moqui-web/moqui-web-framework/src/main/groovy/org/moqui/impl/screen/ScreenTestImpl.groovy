...
class ScreenTestImpl implements ScreenTest {
    ...
    protected final ScreenFacade sfi
    ...
    ScreenTestImpl(WebExecutionContextFactory ecfi) {
        this.ecfi = ecfi
        sfi = ecfi.getScreen()
    ...
    static class ScreenTestRenderImpl implements ScreenTestRender {
        ...
        ScreenTestRender render() {
            ...
            WebExecutionContextFactory ecfi = sti.ecfi
            WebExecutionContext localEci = (WebExecutionContext) ecfi.getExecutionContext()
            String username = localEci.getUser().getUsername()
//            Subject loginSubject = localEci.getUser().getCurrentSubject()
            boolean authzDisabled = localEci.getArtifactExecution().getAuthzDisabled()
            ...
            Thread txThread = Thread.start('ScreenTestRender', {
                try {
                    ...
                    //TODO
//                    if (loginSubject != null) eci.getUser().internalLoginSubject(loginSubject)
//                    else
                    if (username != null && !username.isEmpty()) eci.getUser().internalLoginUser(username)
                    if (authzDisabled) eci.getArtifactExecution().disableAuthz()
                    // as this is used for server-side transition calls don't do tarpit checks
                    eci.getArtifactExecution().disableTarpit()
        ...
        private static void renderInternal(WebExecutionContext eci, ScreenTestRenderImpl stri) {
