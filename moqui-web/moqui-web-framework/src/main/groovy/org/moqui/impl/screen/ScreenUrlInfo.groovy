...
class ScreenUrlInfo {
    ...
    static ScreenUrlInfo getScreenUrlInfo(ScreenRender sri, String url) {
        Cache<String, ScreenUrlInfo> screenUrlCache = sri.getScreenFacade().screenUrlCache
    ...
    static ScreenUrlInfo getScreenUrlInfo(ScreenFacade sfi, String url) {
        Cache<String, ScreenUrlInfo> screenUrlCache = sfi.getScreenUrlCache()
        ScreenUrlInfo cached = (ScreenUrlInfo) screenUrlCache.get(url)
        if (cached != null) return cached
    ...
    static ScreenUrlInfo getScreenUrlInfo(ScreenFacade sfi, ScreenDefinition rootSd, ScreenDefinition fromScreenDef,
    ...
    static ScreenUrlInfo getScreenUrlInfo(ScreenFacade sfi, HttpServletRequest request) {
        String webappName = MoquiWeb.WEBAPP_NAME //request.servletContext.getInitParameter("moqui-name")
    ...
    /** Stub mode for ScreenUrlInfo, represent a plain URL and not a screen URL */
    ScreenUrlInfo(ScreenRender sri, String url) {
        this.sfi = sri.getScreenFacade()
        this.ecfi = sfi.getFactory()
        this.rootSd = sri.getRootScreenDef()
        this.plainUrl = url
    }
    ScreenUrlInfo(ScreenFacade sfi, String url) {
        this.sfi = sfi
        this.ecfi = sfi.getFactory()
        this.plainUrl = url
    }

    ScreenUrlInfo(ScreenFacade sfi, ScreenDefinition rootSd, ScreenDefinition fromScreenDef,
                  ArrayList<String> fpnl, String subscreenPath, int lastStandalone) {
        this.sfi = sfi
        this.ecfi = sfi.getFactory()
        this.rootSd = rootSd
    ...
    boolean isPermitted(ExecutionContext ec, TransitionItem transitionItem) {
        ...
        LinkedList<ArtifactExecutionInfo> artifactExecutionInfoStack = new LinkedList<ArtifactExecutionInfo>()
        ...
        for (int i = 0; i < screenPathDefListSize; i++) {
            ...
            ArtifactExecutionInfo aeii = new ArtifactExecutionInfoImpl(screenDef.getLocation(),
                    ArtifactExecutionInfo.AT_XML_SCREEN, ArtifactExecutionInfo.AUTHZA_VIEW, null)

            ArtifactExecutionInfo lastAeii = (ArtifactExecutionInfo) artifactExecutionInfoStack.peekFirst()
        ...
        if (transitionItem != null && !transitionItem.isReadOnly() && serviceName != null && !serviceName.isEmpty()) {
            ServiceDefinition sd = sfi.getFactory().getService().getServiceDefinition(serviceName)
    ...
    static ArrayList<String> parseSubScreenPath(ScreenDefinition rootSd, ScreenDefinition fromSd, List<String> fromPathList,
                                                String screenPath, Map inlineParameters, ScreenFacade sfi) {
        ...
        EntityList screenPathAliasList = sfi.getFactory().getEntity().find("moqui.screen.ScreenPathAlias")
    ...
    static class UrlInstance {
        ...
        UrlInstance(ScreenUrlInfo sui, ScreenRenderImpl sri, Boolean expandAliasTransition) {
            this.sui = sui
            this.sri = sri
            ec = sri.sfi.getFactory().getExecutionContext() as WebExecutionContext
