...
class WebFacadeImpl implements WebFacade {
    ...
    protected WebExecutionContext eci
    protected WebExecutionContextFactory ecf
    ...
    WebFacadeImpl(String webappMoquiName, HttpServletRequest request, HttpServletResponse response,
        ...
        this.ecf = (WebExecutionContextFactory) eci.getFactory()
        request.setAttribute("ecf", this.ecf)
    ...
    void runFirstHitInVisitActions() {
        WebExecutionContextFactory ecf = (WebExecutionContextFactory) eci.getFactory()
        WebappInfo wi = ecf.getWebappInfo(webappMoquiName)
    ...
    static String getWebappRootUrl(String webappName, String servletContextPath, boolean requireFullUrl, Boolean useEncryption, WebExecutionContext eci) {
    ...
    static String makeWebappHost(String webappName, WebExecutionContext eci, WebFacade webFacade, boolean requireEncryption) {
        WebExecutionContextFactory ecf = (WebExecutionContextFactory) eci.getFactory()
        WebappInfo webappInfo = ecf.getWebappInfo(webappName)
    ...
    static String makeWebappRootUrl(String webappName, String servletContextPath, WebExecutionContext eci, WebFacade webFacade,
    ...
    static void sendJsonResponseInternal(Object responseObj, ExecutionContext eci, HttpServletRequest request,
    ...
    static void sendTextResponseInternal(String text, String contentType, String filename, ExecutionContext eci,
    ...
    static void sendResourceResponseInternal(String location, boolean inline, WebExecutionContext eci, HttpServletResponse response) {
        WebExecutionContextFactory ecf = (WebExecutionContextFactory) eci.getFactory()
        ResourceReference rr = ecf.getResource().getLocationReference(location)
    ...
    @Override void handleXmlRpcServiceCall() { new ServiceXmlRpcDispatcher(eci, ecf.getService().getFactory()).dispatch(request, response) }
    @Override void handleJsonRpcServiceCall() { new ServiceJsonRpcDispatcher(eci, ecf.getService().getFactory()).dispatch() }
        ...
        try {
            ...
            if (parmStack._requestBodyJsonList) {
                ...
                for (Object bodyListObj in parmStack._requestBodyJsonList) {
                    ...
                    Object responseObj = ecf.getEntity().rest(method, extraPathNameList, parmStack, masterNameInPath)
    ...
    void handleSystemMessage(List<String> extraPathNameList) {
        ...
        try {
            ...
            String userId = eci.getUser().getUserId()
            ...
            if (!messageAuthEnumId || "SmatLogin".equals(messageAuthEnumId)) {
                ...
                try {
                    eci.getArtifactExecution().isPermitted(aeii, null, true, false, true, null)
                } catch (ArtifactAuthorizationException e) {
            ...
            if (eci.getMessage().hasError()) {
                response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, eci.getMessage().getErrorsString())
                return
             }
    ...
    void saveMessagesToSession() {
        List<MessageInfo> messageInfos = eci.getMessage().getMessageInfos()
        ...
        List<MessageInfo> publicMessageInfos = eci.getMessage().getPublicMessageInfos()
        ...
        List<String> errors = eci.getMessage().getErrors()
        if (errors != null && errors.size() > 0) session.setAttribute("moqui.message.errors", errors)
        List<ValidationError> validationErrors = eci.getMessage().validationErrors
    ...
    void viewEmailMessage() {
        ...
        try {
            String emailMessageId = (String) eci.getContext().get("emailMessageId")
    ...
    protected DiskFileItemFactory makeDiskFileItemFactory() {
        ...
        File repository = new File(eci.getFactory().runtimePath + "/tmp")
