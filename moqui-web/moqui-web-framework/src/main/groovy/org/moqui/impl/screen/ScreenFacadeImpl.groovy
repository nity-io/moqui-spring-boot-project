...
class ScreenFacadeImpl implements ScreenFacade {
    ...
    protected final WebExecutionContextFactory ecfi
    ...
    ScreenFacadeImpl(WebExecutionContextFactory ecfi) {
        this.ecfi = ecfi
        screenLocationCache = ecfi.getCache().getCache("screen.location", String.class, ScreenDefinition.class)
        screenLocationPermCache = ecfi.getCache().getCache("screen.location.perm", String.class, ScreenDefinition.class)
        screenUrlCache = ecfi.getCache().getCache("screen.url", String.class, ScreenUrlInfo.class)
        screenInfoCache = ecfi.getCache().getCache("screen.info", String.class, Object.class) as Cache<String, List<ScreenInfo>>;
        screenInfoRefRevCache = ecfi.getCache().getCache("screen.info.ref.rev", String.class, Object.class) as Cache<String, Set<String>>;
        screenTemplateModeCache = ecfi.getCache().getCache("screen.template.mode", String.class, Template.class)
        screenTemplateLocationCache = ecfi.getCache().getCache("screen.template.location", String.class, Template.class)
        widgetTemplateLocationCache = ecfi.getCache().getCache("widget.template.location", String.class, MNode.class)
        screenFindPathCache = ecfi.getCache().getCache("screen.find.path", String.class, Object.class) as Cache<String, ArrayList<String>>;
        dbFormNodeByIdCache = ecfi.getCache().getCache("screen.form.db.node", String.class, MNode.class)
    ...
    WebExecutionContextFactory getEcfi() { return ecfi }
    ...
    protected synchronized Template makeTemplateByMode(String renderMode) {
        ...
        try {
            newTemplate = new Template("moqui.automatic.${renderMode}", new StringReader(rootTemplate),
                    ecfi.getResource().getFtlTemplateRenderer().getFtlConfiguration())
        } catch (Exception e) {
    ...
    protected synchronized MNode makeWidgetTemplatesNodeByLocation(String templateLocation) {
        ...
        templatesNode = MNode.parse(templateLocation, ecfi.getResource().getLocationStream(templateLocation))
    ...
    String rootScreenFromHost(String host, String webappName) {
        WebExecutionContextFactoryImpl.WebappInfo webappInfo = ecfi.getWebappInfo(webappName)
    ...
    class TransitionInfo implements Serializable {
        ...
        TransitionInfo(ScreenInfo si, TransitionItem ti) {
            ...
            for (ScreenDefinition.ResponseItem ri in ti.conditionalResponseList) {
                ...
                ScreenUrlInfo sui = ScreenUrlInfo.getScreenUrlInfo(ecfi.getScreen(), si.rootInfo.sd, si.sd, si.screenPath, expandedUrl, 0)
    ...
    @Override
    Cache<String, MNode> getDbFormNodeByIdCache() {
        return dbFormNodeByIdCache
    }
    ...
    @Override
    Cache<String, ScreenDefinition> getScreenLocationCache() {
        return screenLocationCache
    }

    @Override
    Cache<String, ScreenUrlInfo> getScreenUrlCache() {
        return screenUrlCache
    }

    @Override
    Cache<String, ArrayList<String>> getScreenFindPathCache() {
        return screenFindPathCache
    }
}
