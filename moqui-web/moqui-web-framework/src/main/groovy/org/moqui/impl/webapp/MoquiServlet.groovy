...
class MoquiServlet extends HttpServlet {
    ...
    void service(HttpServletRequest request, HttpServletResponse response) {
        WebExecutionContextFactory ecfi = (WebExecutionContextFactory) getServletContext().getAttribute("executionContextFactory")
        String webappName = MoquiWeb.WEBAPP_NAME
        ...
        WebExecutionContext webExecutionContext = WebExecutionContextFactoryImpl.activeWebContext.get()
        if(webExecutionContext != null){
            webExecutionContext.destroy()
        }

        ExecutionContext activeEc = ExecutionContextThreadHolder.activeContext.get()
        if (activeEc != null) {
    ...
    static void sendErrorResponse(HttpServletRequest request, HttpServletResponse response, int errorCode, String errorType,
                                  String message, Throwable origThrowable, WebExecutionContextFactory ecfi, String moquiWebappName, ScreenRenderImpl sri) {
        ...
        if (errorScreenNode != null) {
            try {
                ...
                ec.getScreen().makeRender().webappName(moquiWebappName).renderMode("html")
