...
class ResourceFacadeImpl implements ResourceFacade {
    ...
    protected final ResourceExecutionContextFactoryImpl ecfi
    ...
    ResourceFacadeImpl(ResourceExecutionContextFactoryImpl ecfi) {
        this.ecfi = ecfi
    ...
    ResourceExecutionContextFactoryImpl getEcfi() { ecfi }
    ...
    @Override Object script(String location, String method) {
        ...
        if (sr != null) {
            ...
            return JavaxScriptRunner.bindAndRun(location, ec, engine, ecfi.cacheFacade.getCache("resource.script${extension}.location"), this)
        }
    }
...
//                else cs.push(additionalContext)
    ...
    Object setInContext(String field, String from, String value, String defaultValue, String type, String setIfEmpty) {
        ...
        ecfi.getEci().getContext().put("_tempValue", tempValue)
    ...
    protected boolean conditionInternal(String expression, String debugLocation, ExecutionContext ec) {
...
//                else cs.push(additionalContext)
    ...
    @Override Object expression(String expression, String debugLocation) {
        ExecutionContext ec = ecfi.getEci()
        return expressionInternal(expression, debugLocation, ec)
    }

    @Override Object expression(String expression, String debugLocation, ExecutionContext ec) {
        return expressionInternal(expression, debugLocation, ec)
    }

    protected Object expressionInternal(String expression, String debugLocation, ExecutionContext ec) {
...
//                else cs.push(additionalContext)
    ...
    @Override String expand(String inputString, String debugLocation, Map additionalContext, boolean localize) {
        ...
        if (doPushPop) cs = eci.getContext()
        try {
            if (doPushPop) {
//                if (additionalContext instanceof EntityValueBase) { cs.push(((EntityValueBase) additionalContext).getValueMap()) }
//                else { cs.push(additionalContext) }
    ...
    Script getGroovyScript(String expression, ExecutionContext ec) {
        ContextBinding curBinding = ec.getContextBinding()
    ...
    Integer xslFoTransform(StreamSource xslFoSrc, StreamSource xsltSrc, OutputStream out, String contentType) {
        ...
        ExecutionContext ec = ecfi.getEci()
        ExecutionContextImpl.ThreadPoolRunnable runnable = new ExecutionContextImpl.ThreadPoolRunnable(ec, {
    ...
    static class LocalResolver implements URIResolver {
        ...
        LocalResolver(ResourceExecutionContextFactoryImpl ecfi, URIResolver defaultResolver) {
            this.ecfi = ecfi
    ...
    @Override
    ResourceExecutionContextFactory getFactory(){
        return ecfi
    }

    @Override
    ScriptRunner getXmlActionsScriptRunner() {
