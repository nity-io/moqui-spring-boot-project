...
public class RemoteJsonRpcServiceRunner implements ServiceRunner {
    ...
    public Map<String, Object> runService(ServiceDefinition sd, Map<String, Object> parameters) {
        ...
        return runJsonService(sd.serviceNameNoHash, location, method, parameters, ec, ecfi)
    }

    static Map<String, Object> runJsonService(String serviceName, String location, String method,
                                              Map<String, Object> parameters, ExecutionContext ec, ServiceExecutionContextFactoryImpl ecfi) {
        ...
        try {
            ...
            String errMsg = ecfi.resource.expand('Error parsing JSON-RPC response for service [${serviceName ?: method}]: ${t.toString()}','',[serviceName:serviceName, method:method, t:t])
        ...
        if (jsonObj instanceof Map) {
            ...
            if (responseMap.error) {
                ...
                ec.message.addError((String) ((Map) responseMap.error)?.message ?: ecfi.resource.expand('JSON-RPC error with no message, code [${responseMap.error?.code}]','',[responseMap:responseMap]))
                return null
            } else {
            ...
            String errMsg = ecfi.resource.expand('JSON-RPC response was not a object/Map for service [${serviceName ?: method}]: ${jsonObj}','',[serviceName:serviceName,method:method,jsonObj:jsonObj])
            logger.error(errMsg)
            ec.message.addError(errMsg)
            return null
        }
