...
class ScheduledJobRunner implements Runnable {
    ...
    private final ServiceExecutionContextFactoryImpl ecfi
    ...
    ScheduledJobRunner(ServiceExecutionContextFactoryImpl ecfi) {
        this.ecfi = ecfi
    }
        ...
        try {
            ...
            for (int i = 0; i < serviceJobListSize; i++) {
                ...
                if (serviceJob.repeatCount != null) {
                    ...
                    EntityFind entityFind = efi.find("moqui.service.job.ServiceJobRun").condition("jobName", jobName).useCache(false)
                    long runCount = entityFind.count()
