...
class EmailEcaRule {
    ...
    EmailEcaRule(ServiceExecutionContextFactoryImpl ecfi, MNode emecaNode, String location) {
        ...
        if (emecaNode.hasChild("condition") && emecaNode.first("condition").children) {
            ...
            condition = new XmlAction(ecfi.resourceFacade.getEcfi(), emecaNode.first("condition").children.get(0), location + ".condition")
        }
        // prep actions
        if (emecaNode.hasChild("actions")) {
            actions = new XmlAction(ecfi.resourceFacade.getEcfi(), emecaNode.first("actions"), location + ".actions")
        }
    }
