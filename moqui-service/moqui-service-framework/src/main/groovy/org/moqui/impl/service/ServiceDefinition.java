...
public class ServiceDefinition {
    ...
    public final ServiceExecutionContextFactoryImpl ecfi;
    ...
    public ServiceDefinition(ServiceFacadeImpl sfi, String path, MNode sn, ServiceExecutionContextFactoryImpl ecfi) {
        this.sfi = sfi;
        this.ecfi = ecfi;
        ...
        if (serviceNode.hasChild("actions")) {
            xmlAction = new XmlAction(sfi.ecfi.resourceFacade.getEcfi(), serviceNode.first("actions"), serviceName);
        } else {
            xmlAction = null;
        }
    ...
    public Map<String, Object>  convertValidateCleanParameters(Map<String, Object> parameters, ExecutionContext eci) {
    ...
    private Map<String, Object> nestedParameterClean(String namePrefix, Map<String, Object> parameters,
                                      ParameterInfo[] parameterInfoArray, ExecutionContext eci) {
            ...
            if (parameterIsEmpty) {
                if (parameterInfo.hasDefault) {
                    ...
                    if (parameterInfo.defaultStr != null) {
                        ...
                        parameterValue = ecfi.getResource().expression(parameterInfo.defaultStr, null, combinedMap);
                        if (parameterValue != null) {
                            hasParameter = true;
                            isString = false;
                            isCollection = false;
                        ...
                        if (parameterInfo.defaultValueNeedsExpand) {
                            Map<String, Object> combinedMap = new HashMap<>(parameters);
                            combinedMap.putAll(newMap);
                            stringValue = ecfi.getResource().expand(parameterInfo.defaultValue, null, combinedMap, false);
                        } else {
                ...
                if (parameterIsEmpty && validate && parameterInfo.required)
                    eci.getMessage().addValidationError(null, namePrefix + parameterName, serviceName, eci.getL10n().localize("Field cannot be empty"), null);
            }
                    ...
                    if (parameterInfo.validationNodeList != null) {
                        ...
                        for (int valIdx = 0; valIdx < valListSize; valIdx++) {
                            ...
                            try {
                                ...
                                eci.getMessage().addValidationError(null, parameterName, serviceName, ecfi.getResource().expand("Value entered failed ${valNode.name} validation: ${t.message}", "", map), null);
                            }
                        }
                    }
                }
    ...
    private boolean validateParameterSingle(MNode valNode, String parameterName, Object pv, ExecutionContext eci) {
        ...
        if ("val-or".equals(validateName)) {
            ...
            if (!(pv instanceof CharSequence)) {
                ...
                eci.getMessage().addValidationError(null, parameterName, serviceName,
                        ecfi.getResource().expand("Value entered (${pv}) is not a string, cannot do matches validation.", "", map), null);
                return false;
            }
            ...
            if (regexp != null && !regexp.isEmpty() && !pvString.matches(regexp)) {
                ...
                eci.getMessage().addValidationError(null, parameterName, serviceName, ecfi.getResource().expand(message != null && !message.isEmpty() ? message : "Value entered (${pv}) did not match expression: ${regexp}", "", map), null);
                return false;
            }
            ...
            if (minStr != null && !minStr.isEmpty()) {
                ...
                if ("false".equals(valNode.attribute("min-include-equals"))) {
                    if (bdVal.compareTo(min) <= 0) {
                        ...
                        eci.getMessage().addValidationError(null, parameterName, serviceName,
                                ecfi.getResource().expand("Value entered (${pv}) is less than or equal to ${min} must be greater than.", "", map), null);
                        return false;
                    }
                } else {
                    if (bdVal.compareTo(min) < 0) {
                        Map<String, Object> map = new HashMap<>(2); map.put("pv", pv); map.put("min", min);
                        eci.getMessage().addValidationError(null, parameterName, serviceName,
                                ecfi.getResource().expand("Value entered (${pv}) is less than ${min} and must be greater than or equal to.", "", map), null);
                        return false;
                    }
                }
            }
            ...
            if (maxStr != null && !maxStr.isEmpty()) {
                ...
                if ("true".equals(valNode.attribute("max-include-equals"))) {
                    if (bdVal.compareTo(max) > 0) {
                        ...
                        eci.getMessage().addValidationError(null, parameterName, serviceName, ecfi.getResource().expand("Value entered (${pv}) is greater than ${max} and must be less than or equal to.", "", map), null);
                        return false;
                    }
                    ...
                    if (bdVal.compareTo(max) >= 0) {
                        Map<String, Object> map = new HashMap<>(2); map.put("pv", pv); map.put("max", max);
                        eci.getMessage().addValidationError(null, parameterName, serviceName, ecfi.getResource().expand("Value entered (${pv}) is greater than or equal to ${max} and must be less than.", "", map), null);
                        return false;
                    }
                }
            }
            ...
            try {
                ...
                Map<String, Object> map = new HashMap<>(1); map.put("pv", pv);
                eci.getMessage().addValidationError(null, parameterName, serviceName, ecfi.getResource().expand("Value [${pv}] is not a whole (integer) number.", "", map), null);
                return false;
            }
            ...
            try {
                ...
                map.put("pv", pv);
                eci.getMessage().addValidationError(null, parameterName, serviceName, ecfi.getResource().expand("Value [${pv}] is not a decimal number.", "", map), null);
                return false;
            }
            ...
            if (minStr != null && !minStr.isEmpty()) {
                ...
                if (str.length() < min) {
                    ...
                    eci.getMessage().addValidationError(null, parameterName, serviceName, ecfi.getResource().expand("Value entered (${pv}), length ${str.length()}, is shorter than ${minStr} characters.", "", map), null);
                    return false;
                }
            ...
            if (maxStr != null && !maxStr.isEmpty()) {
                ...
                if (str.length() > max) {
                    ...
                    eci.getMessage().addValidationError(null, parameterName, serviceName, ecfi.getResource().expand("Value entered (${pv}), length ${str.length()}, is longer than ${maxStr} characters.", "", map), null);
                    return false;
                }
            }
            ...
            if (!emailValidator.isValid(str)) {
                ...
                eci.getMessage().addValidationError(null, parameterName, serviceName, ecfi.getResource().expand("Value entered (${str}) is not a valid email address.", "", map), null);
                return false;
            }
            ...
            if (!urlValidator.isValid(str)) {
                Map<String, Object> map = new HashMap<>(1); map.put("str", str);
                eci.getMessage().addValidationError(null, parameterName, serviceName, ecfi.getResource().expand("Value entered (${str}) is not a valid URL.", "", map), null);
                return false;
            }
            ...
            for (char c : str.toCharArray()) {
                if (!Character.isLetter(c)) {
                    Map<String, Object> map = new HashMap<>(1); map.put("str", str);
                    eci.getMessage().addValidationError(null, parameterName, serviceName, ecfi.getResource().expand("Value entered (${str}) must have only letters.", "", map), null);
                    return false;
                }
            }
            ...
            for (char c : str.toCharArray()) {
                if (!Character.isDigit(c)) {
                    Map<String, Object> map = new HashMap<>(1); map.put("str", str);
                    eci.getMessage().addValidationError(null, parameterName, serviceName, ecfi.getResource().expand("Value [${str}] must have only digits.", "", map), null);
                    return false;
                }
            }
            ...
            if (after != null && !after.isEmpty()) {
                ...
                if (cal != null && cal.compareTo(compareCal) < 0) {
                    ...
                    eci.getMessage().addValidationError(null, parameterName, serviceName, ecfi.getResource().expand("Value entered (${pv}) is before ${after}.", "", map), null);
                    return false;
                }
            }
            ...
            if (before != null && !before.isEmpty()) {
                ...
                if (cal != null && cal.compareTo(compareCal) > 0) {
                    Map<String, Object> map = new HashMap<>(1); map.put("pv", pv);
                    eci.getMessage().addValidationError(null, parameterName, serviceName, ecfi.getResource().expand("Value entered (${pv}) is after ${before}.", "", map), null);
                    return false;
                }
            }
            ...
            if (!ccv.isValid(str)) {
                Map<String, Object> map = new HashMap<>(1); map.put("str", str);
                eci.getMessage().addValidationError(null, parameterName, serviceName, ecfi.getResource().expand("Value entered is not a valid credit card number.", "", map), null);
                return false;
            }
