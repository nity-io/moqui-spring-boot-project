...
public class ServiceJsonRpcDispatcher {
    ...
    private final ExecutionContext eci
    private final ServiceExecutionContextFactory ecfi

    public ServiceJsonRpcDispatcher(ExecutionContext eci, ServiceExecutionContextFactory ecfi) {
        this.eci = eci
        this.ecfi = ecfi
    }
    ...
    protected Map callSingle(String method, Object paramsObj, Object id) {
        ...
        ServiceDefinition sd = method ? ecfi.getService().getServiceDefinition(method) : null
        if (!method) {
        ...
        if (errorMessage == null) {
            try {
                result = ecfi.service.sync().name(sd.serviceName).parameters((Map) paramsObj).call()
