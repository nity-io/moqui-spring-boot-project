...
class ServiceEcaRule {
    ...
    ServiceEcaRule(ServiceExecutionContextFactoryImpl ecfi, MNode secaNode, String location) {
        ...
        if (secaNode.hasChild("condition") && secaNode.first("condition").children) {
            ...
            condition = new XmlAction(ecfi.resourceFacade.getEcfi(), secaNode.first("condition").children.get(0), location + ".condition")
        } else {
            condition = (XmlAction) null
        }
        // prep actions
        if (secaNode.hasChild("actions")) {
            actions = new XmlAction(ecfi.resourceFacade.getEcfi(), secaNode.first("actions"), null) // was location + ".actions" but no unique!
        } else {
    ...
    void registerTx(String serviceName, Map<String, Object> parameters, Map<String, Object> results, ServiceExecutionContextFactoryImpl ecfi) {
        if (!this.serviceNameNoHash.equals(serviceName)) return
    ...
    static class SecaSynchronization implements Synchronization {
        ...
        protected ServiceExecutionContextFactoryImpl ecfi
        ...
        SecaSynchronization(ServiceEcaRule sec, Map<String, Object> parameters, Map<String, Object> results, ServiceExecutionContextFactoryImpl ecfi) {
            this.ecfi = ecfi
            this.sec = sec
